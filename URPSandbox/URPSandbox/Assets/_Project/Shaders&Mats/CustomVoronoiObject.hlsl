                                        ...
                                        out float CellValue)
{
    ...
    if (YFade)
    {
        CellValue = (cell + nearestCell).y - YHeight;
       // CellValue = YHeight; //       DistanceSquared((cell + nearestCell) / CellDensity, float3(0, YHeight, 0)); //        YHeight; //(cell + nearestCell) - YHeight; // * YHeight * YPower / CellDensity; // * lerp(hash(cell + nearestCell), .5, .6); // ;
